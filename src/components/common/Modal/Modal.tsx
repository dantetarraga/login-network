...
import cls from 'components/common/Modal/Modal.module.scss';
...
export class Modal extends Component<ModalPropsType, StateType> {
	...
	render() {
		...
		const bcgStyles = classNames(cls.background, {
			[cls.bcgActive]: this.state.mountingDone
		});
		const wndStyles = classNames(cls.window, {
			[cls.wndActive]: this.state.mountingDone
		});
		...
		return (
			<Portal>
				<div className={cls.modalWrapper}>
					<div className={bcgStyles} onClick={this.closeHandler.bind(this)}></div>
					<div className={wndStyles}>
						{this.props.children}
					</div>
				</div>
			</Portal>
